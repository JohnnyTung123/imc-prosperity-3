...
class Trader:
    def run(self, state: TradingState):
        ...
        sell_limits = {}
        for S in symbols:
            buy_limits[S] = position_limits[S] - cur_pos[S]
            sell_limits[S] = position_limits[S] + cur_pos[S]
            result[S] = []
            
        for S in {SQI}:
            # market trade to neutralize
            if (cur_pos[S] > 0):
                result[S].append(Order(S, round(avg_prices[S]), -cur_pos[S]))
                sell_limits[S] -= cur_pos[S]
            elif (cur_pos[S] < 0):
                result[S].append(Order(S, round(avg_prices[S]), -cur_pos[S]))
                buy_limits[S] += cur_pos[S]
            # limit trade to profit
            S_bid_price = math.floor(bid_prices[S]) + 1
            S_ask_price = math.ceil(ask_prices[S]) - 1
            if (S_bid_price < S_ask_price):
                result[S].extend([Order(S, S_bid_price, buy_limits[S]), Order(S, S_ask_price, -sell_limits[S])])
