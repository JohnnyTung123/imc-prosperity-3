...
class Trader:
    def run(self, state: TradingState):
        ...
        for S in {RAR, KLP}:
         ...
         # --- Olivia-following Strategy for SQUID_INK, KELP, CROISSANTS ---
        for sym in [SQI, CST]:
            orders = []
            ...
            # get current position
            position = 0
            if (sym in state.position):
                position = state.position[sym]
                
            max_buy = position_limits[sym] - position
            max_sell = position_limits[sym] + position
            ...
            for trade in state.market_trades.get(sym, []):
                if trade.buyer == "Olivia" and trade.seller != "Olivia" and buy_price is not None:
                    buy_qty = max_buy
                elif trade.seller == "Olivia" and trade.buyer != "Olivia" and sell_price is not None:
                    sell_qty = max_sell
                    
            orders = []
            if buy_qty > 0:
                orders.append(Order(sym, buy_price, buy_qty))
                print(f"[{sym}] Following Olivia BUY: {buy_qty} @ {buy_price}")
            if sell_qty > 0:
                orders.append(Order(sym, sell_price, -sell_qty))
                print(f"[{sym}] Following Olivia SELL: {sell_qty} @ {sell_price}")
